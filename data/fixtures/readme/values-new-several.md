The mod keeps a separate `CameraUnlock.ini` at each of these paths in the game folder:

- `A\CameraUnlock.ini`
- `B\CameraUnlock.ini` or `C\CameraUnlock.ini`

Where a line names two paths, which one is used depends on the store the game came from. The mod creates each file when it starts and finds none. Edit them with any text editor.
