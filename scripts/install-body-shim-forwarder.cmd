@echo off
:: ============================================
:: CameraUnlock forwarding-shim install body (shared)
:: ============================================
:: Source of truth: cameraunlock-core/scripts/install-body-shim-forwarder.cmd.
:: Per-mod install.cmd wrappers set the CONFIG BLOCK + WRAPPER_DIR and
:: `call` here.
::
:: A shim whose non-intercepted exports are linker forwards to a RENAMED copy
:: of the system DLL it replaces (opengl32.dll forwarding to
:: opengl32_real.dll). install-body-shim.cmd cannot install one: it copies the
:: shim and nothing else, and the loader then fails to resolve every forward,
:: so the game does not start. That copy is not redistributable, so it cannot
:: ship in plugins\ either. This body deploys the shim exactly as the shim
:: body does, then copies SYSTEM_DLL out of the user's own system directory
:: to EXE_DIR\SYSTEM_DLL_COPY.
::
:: The copy is refreshed on every install rather than kept when present, and
:: never backed up. It is a copy of a Windows file that Windows Update
:: replaces; keeping an old one pins the game to a stale system DLL, and
:: backing one up would record our own stale copy as something to restore.
::
:: SYSTEM_DLL_ARCH names the game's bitness, not this cmd.exe's. A 32-bit
:: host (a 32-bit launcher, say) sees System32 redirected to SysWOW64, so the
:: native directory is reached through Sysnative when it exists.
::
:: FRAMEWORK_TYPE is "None" on the state file: there is still no framework.
:: The uninstall wrapper lists SYSTEM_DLL_COPY in MOD_DLLS, and the None
:: dispatch deletes it there (no .backup is ever made for it).
::
...
::   SYSTEM_DLL               system DLL the shim replaces, e.g. opengl32.dll
::   SYSTEM_DLL_COPY          name the shim's forwards target, e.g. opengl32_real.dll
::   SYSTEM_DLL_ARCH          x64 or x86, the game executable's architecture
...
for %%v in (GAME_ID MOD_DISPLAY_NAME MOD_INTERNAL_NAME STATE_FILE FRAMEWORK_TYPE MOD_DLLS SHIM_MARKER SYSTEM_DLL SYSTEM_DLL_COPY SYSTEM_DLL_ARCH) do (
    if not defined %%v (
        echo ERROR: %%v is not set in this script's CONFIG BLOCK.
        exit /b 1
    )
)
:: The same name twice would copy the system DLL over the shim just deployed,
:: and the game would run without the mod while the install reported success.
if /i "%SYSTEM_DLL%"=="%SYSTEM_DLL_COPY%" (
    echo ERROR: SYSTEM_DLL and SYSTEM_DLL_COPY are both "%SYSTEM_DLL%" in this script's CONFIG BLOCK.
    exit /b 1
)
set "SYSTEM_DLL_DIR="
if /i "%SYSTEM_DLL_ARCH%"=="x64" (
    if exist "%SystemRoot%\Sysnative\%SYSTEM_DLL%" (
        set "SYSTEM_DLL_DIR=%SystemRoot%\Sysnative"
    ) else (
        set "SYSTEM_DLL_DIR=%SystemRoot%\System32"
    )
)
if /i "%SYSTEM_DLL_ARCH%"=="x86" (
    if exist "%SystemRoot%\SysWOW64\" (
        set "SYSTEM_DLL_DIR=%SystemRoot%\SysWOW64"
    ) else (
        set "SYSTEM_DLL_DIR=%SystemRoot%\System32"
    )
)
if not defined SYSTEM_DLL_DIR (
    echo ERROR: SYSTEM_DLL_ARCH is "%SYSTEM_DLL_ARCH%" in this script's CONFIG BLOCK; it must be x64 or x86.
    exit /b 1
)
...
:: Before the shim, never after: a shim whose forward target is missing stops
:: the game starting at all, while a system DLL copy with no shim beside it is
:: never loaded by anything.
if not exist "!SYSTEM_DLL_DIR!\%SYSTEM_DLL%" (
    echo   ERROR: !SYSTEM_DLL_DIR!\%SYSTEM_DLL% not found - this mod needs it to forward to.
    set "DEPLOY_FAILED=1"
    goto :deploy_done
)
copy /y "!SYSTEM_DLL_DIR!\%SYSTEM_DLL%" "!EXE_DIR!\%SYSTEM_DLL_COPY%" >nul
if errorlevel 1 (
    echo   ERROR: Failed to copy %SYSTEM_DLL% to %SYSTEM_DLL_COPY% - is the game folder writable?
    set "DEPLOY_FAILED=1"
    goto :deploy_done
)
echo   Copied !SYSTEM_DLL_DIR!\%SYSTEM_DLL% to %SYSTEM_DLL_COPY%
