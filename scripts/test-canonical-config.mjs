...
const mutations = [
  ["an LF line", replace("UdpPort=4242\r\n", "UdpPort=4242\n"), "line 11 does not end in CRLF"],
  ["no final CRLF", base.slice(0, -2), "line 84 does not end in CRLF"],
  ["LF throughout", base.replace(/\r\n/g, "\n"), "lines 1, 2, 3 and 81 more do not end in CRLF"],
  ["two non-ASCII lines", replace("; UDP port", "; UDP p\xF6rt").replace("; true: head", "; tr\xFCe: head"), "lines 10, 14 hold a byte above 0x7F"],
  ["a UTF-8 mark", `\xEF\xBB\xBF${base}`, "starts with a UTF-8 byte order mark"],
  ["a non-ASCII byte", replace("; UDP port", "; UDP p\xF6rt"), "line 10 holds a byte above 0x7F"],
  ["a NUL byte", replace("UdpPort=4242", "UdpPort=42\x0042"), "is unreadable: line 11 holds a NUL byte"],
  ["a UTF-16 mark", `\xFF\xFE${base}`, "is unreadable: it starts with a UTF-16 byte order mark"],
  ["spaces around =", replace("UdpPort=4242", "UdpPort = 4242"), "line 11 is not written Key=value"],
  ["an indented key", replace("UdpPort=4242", "  UdpPort=4242"), "line 11 is not written Key=value"],
  ["an indented header", replace("[Network]", "  [Network]"), "line 9 is not written [Network]"],
  ["spaces inside a header", replace("[Network]", "[ Network ]"), "line 9 is not written [Network]"],
  ["a repeated section", append(["[hotkeys]", "ZoomKey=F5"]), "line 86: [hotkeys] repeats the section begun on line 69"],
  ["a note after a header", replace("[Network]", "[Network] ; port"), "line 9: \"; port\" follows [Network]"],
  ["an unclosed header", replace("[Network]", "[Network"), ["line 9: \"[Network\" has no closing ]", "line 11: UdpPort is above every section header"]],
  ["a line with no =", replace("UdpPort=4242", "UdpPort"), "line 11: \"UdpPort\" is neither a setting"],
  ["a repeated key", replace("UdpPort=4242", "UdpPort=4242\r\nUdpPort=4243"), "[Network] UdpPort is set on lines 11, 12"],
  ["no ConfigFormat", replace("ConfigFormat=1\r\n", ""), "line 5: [CameraUnlock] has no ConfigFormat"],
  ["ConfigFormat newer", replace("ConfigFormat=1", "ConfigFormat=2"), "line 7: ConfigFormat=2 is newer than format 1"],
  ["ConfigFormat padded", replace("ConfigFormat=1", "ConfigFormat=01"), "line 7: ConfigFormat=01 is written ConfigFormat=1"],
  ["ConfigFormat miscased", replace("ConfigFormat=1", "configformat=1"), "line 7: configformat is spelled ConfigFormat"],
  ["a mod key in the stamp", replace("ConfigFormat=1", "ConfigFormat=1\r\nContract=2"), "line 8: [CameraUnlock] Contract is not written by a mod"],
  ["the stamp miscased", replace("[CameraUnlock]", "[cameraunlock]"), "line 5: [cameraunlock] is spelled [CameraUnlock]"],
  ["a schema section miscased", replace("[Network]", "[network]"), "line 9: [network] is spelled [Network]"],
  ["an alias", replace("UdpPort=4242", "Port=4242"), "line 11: [Network] Port is an alias of [Network] UdpPort"],
  ["a concept miscased", replace("UdpPort=4242", "udpport=4242"), "line 11: [Network] udpport is spelled UdpPort"],
  ["a concept with an underscore", replace("UdpPort=4242", "Udp_Port=4242"), "line 11: [Network] Udp_Port is spelled UdpPort"],
  ...
  ["a deadzone", append(["[Camera]", "YawDeadzone=0.5"]), "line 87: [Camera] YawDeadzone: The mod applies the head pose as the tracker sends it, with no deadzone of its own."],
  ["a deadzone in snake case", append(["[Camera]", "deadzone_deg=0.5"]), "line 87: [Camera] deadzone_deg: The mod applies the head pose as the tracker sends it, with no deadzone"],
  ...
  ["a deadzone minimum", append(["[Rotation]", "DeadzoneMin=0.1"]), "line 87: [Rotation] DeadzoneMin: The mod applies the head pose as the tracker sends it, with no deadzone of its own."],
  ["a curve strength", append(["[Rotation]", "CurveStrength=0.5"]), "line 87: [Rotation] CurveStrength: The mod applies the head pose as the tracker sends it, with no response curve of its own."],
  ["a rotation scale", append(["[Tuning]", "rot_scale=1.5"]), "line 87: [Tuning] rot_scale: The mod applies the head pose as the tracker sends it, with no sensitivity of its own."],
  ["a yaw gain", append(["[ExtendedView]", "YawGain=1.5"]), "line 87: [ExtendedView] YawGain: The mod applies the head pose as the tracker sends it, with no sensitivity of its own."],
  ["a rotation sensitivity", append(["[Tracking]", "RotationSensitivity=1.0"]), "line 87: [Tracking] RotationSensitivity: The mod applies the head pose as the tracker sends it, with no sensitivity of its own."],
  ...
  ["[Deadzone] with a bare Yaw", append(["[Deadzone]", "Yaw=0.5"]), ["line 86: [Deadzone] holds no canonical setting", "line 87: [Deadzone] Yaw: The mod applies the head pose as the tracker sends it, with no deadzone of its own."]],
  ["[Deadzone] with any key", append(["[Deadzone]", "; Degrees.", "Threshold=0.5"]), ["line 86: [Deadzone] holds no canonical setting", "line 88: [Deadzone] Threshold: The mod applies the head pose as the tracker sends it, with no deadzone of its own."]],
  ["[deadzone] miscased", append(["[deadzone]", "YawDegrees=0.5"]), ["line 86: [deadzone] is not a PascalCase name", "line 86: [deadzone] holds no canonical setting", "line 87: [deadzone] YawDegrees: The mod applies the head pose as the tracker sends it, with no deadzone of its own."]],
  ["[Reticle]", append(["[Reticle]"]), "line 86: [Reticle] holds no canonical setting"],
  ["a bare noun", append(["[Flashlight]", "Enabled=true"]), "line 87: [Flashlight] Enabled: a bare Enabled"],
  ["a chord switch", replace("EnableOnStartup=true", "EnableOnStartup=true\r\nChordToggle=true"), "line 16: [General] ChordToggle: a chord is an item of its action's key list"],
  ...
  ["a snake_case key", append(["[Camera]", "yaw_limit=90"]), "line 87: [Camera] yaw_limit is not a PascalCase name"],
  ["a lower-case section", append(["[target]", "CbSize=4"]), "line 86: [target] is not a PascalCase name"],
  ["a key in two sections", append(["[Camera]", "Offset=1", "[Debug]", "Offset=2"]), "line 89: [Debug] Offset: Offset is also a key under [Camera]"],
