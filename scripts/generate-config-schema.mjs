...
// and the C++ test expectation for data/pipeline-conformance.json's preference_modes.
//
    ...
    if (check) {
        if (stale > 0) {
            console.error(`\n${stale} generated file(s) do not match data/config-schema.json ` +
                'and data/pipeline-conformance.json.');
        ...
        console.log(`config schema is in sync (${entries.length} aliases over ${schema.concepts.length} concepts, ` +
            `${trackingModes.modes.length} tracking modes)`);
    }
}
