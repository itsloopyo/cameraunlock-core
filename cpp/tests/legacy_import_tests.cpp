...
// (LegacyVirtualKeyToBindings) and N2 (LegacyFiniteOrDefault), and a LegacyImport over a Config.
...
void TestImportResult() {
    ...
    Check(static_cast<int>(DropRule::KeyCodeOutOfRange) == 1 && static_cast<int>(DropRule::NonFiniteNumber) == 2 &&
              static_cast<int>(DropRule::PoseShaping) == 3 && static_cast<int>(DropRule::Reticle) == 4 &&
              static_cast<int>(DropRule::FollowsDefault) == 5,
...
void TestN1() {
    ...
    LegacyVirtualKeyToBindings(0xFF, "Hotkeys", "PositionToggleKey", dropped);
    LegacyVirtualKeyToBindings(-1, "General", "CycleKey", dropped);
    LegacyVirtualKeyToBindings(LLONG_MIN, "General", "Min", dropped);
    Check(dropped.size() == 4 && SameDrop(dropped[0], DropRule::KeyCodeOutOfRange, "Hotkeys", "ToggleKey", "0x230") &&
              SameDrop(dropped[1], DropRule::KeyCodeOutOfRange, "Hotkeys", "PositionToggleKey", "0xFF") &&
...
void TestLegacyImport() {
    ...
    Check(result.status == ImportStatus::Imported && config.toggle_key.empty() && config.remote_smoothing == 0.15f &&
          ...
          "an import maps through N1 and N2 and returns what it dropped");
