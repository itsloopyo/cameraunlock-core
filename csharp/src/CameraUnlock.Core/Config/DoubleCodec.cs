using System;
using System.Globalization;
using System.Text;

namespace CameraUnlock.Core.Config
{
    /// <summary>
    /// A double, written for people. Of the <c>ToString("G" + N)</c> texts (N = 1 to 17,
    /// invariant culture, 'E' lower-cased) that read back to the same bits, the one with the
    /// smallest N that has no exponent, or, when every one has an exponent, the one with the
    /// smallest N; then ".0" appended when the text has neither '.' nor 'e'. So 1 is
    /// <c>1.0</c>, 10 is <c>10.0</c> rather than <c>1e+01</c>, 0.15 is <c>0.15</c>, 0.00001 is
    /// <c>1e-05</c> and -0 is <c>-0.0</c>, as the C++ codec writes them.
    /// <para>
    /// Reads -?[0-9]+(\.[0-9]+)?([eE][+-]?[0-9]+)? through double.Parse with the invariant
    /// culture, within the codec's inclusive range. No inf, nan, leading '+', leading or
    /// trailing '.', comma or hex float. A number too large for a double is invalid, and so is
    /// one that is not zero but reads as zero, as std::from_chars reports both out of range.
    /// </para>
    /// <para>
    /// A value always reads back to the same bits on the runtime that wrote it. On .NET
    /// Framework, double.Parse is not correctly rounded, so a text written there can be shorter
    /// than the C++ codec's and read in C++ as a neighbouring double, and a C++ text can read
    /// there as a neighbouring double. .NET 8 agrees with C++.
    /// </para>
    /// </summary>
    public sealed class DoubleCodec : IValueCodec<double>
    {
        private const double NegativeZero = -0.0;

        /// <summary>Every finite double.</summary>
        public DoubleCodec() : this(double.MinValue, double.MaxValue)
        {
        }

        /// <exception cref="ArgumentException">A bound is not finite, or <paramref name="min"/> is above <paramref name="max"/>.</exception>
        public DoubleCodec(double min, double max)
        {
            if (double.IsNaN(min) || double.IsInfinity(min) || double.IsNaN(max) || double.IsInfinity(max) || min > max)
            {
                throw new ArgumentException("double codec range needs finite bounds with min not above max", "min");
            }
            Min = min;
            Max = max;
        }

        /// <summary>The smallest value read.</summary>
        public double Min { get; }

        /// <summary>The largest value read.</summary>
        public double Max { get; }

        /// <inheritdoc/>
#if NULLABLE_ENABLED
        public bool TryParse(byte[] text, out double value, out string? error)
#else
        public bool TryParse(byte[] text, out double value, out string error)
#endif
        {
            if (text == null) throw new ArgumentNullException("text");

            value = 0.0;
            error = null;
            bool fullRange = Bits(Min) == Bits(double.MinValue) && Bits(Max) == Bits(double.MaxValue);
            if (!CodecText.MatchesNumberGrammar(text))
            {
                error = fullRange ? "expected a number such as 1.0, 0.15 or 1e-05" : RangeExpectation();
                return false;
            }

            string s = Encoding.ASCII.GetString(text);
            double read;
            if (!TryRead(s, out read))
            {
                error = fullRange ? "expected a number from " + RenderFinite(double.MinValue) + " to "
                    + RenderFinite(double.MaxValue) : RangeExpectation();
                return false;
            }
            if (read == 0.0 && CodecText.HasNonZeroDigit(s))
            {
                error = Min > 0.0 || Max < 0.0 ? RangeExpectation()
                    : "expected 0.0 or a number no closer to zero than " + RenderFinite(double.Epsilon);
                return false;
            }
            if (read < Min || read > Max)
            {
                error = RangeExpectation();
                return false;
            }
            value = read;
            return true;
        }

        /// <inheritdoc/>
        /// <exception cref="ArgumentOutOfRangeException"><paramref name="value"/> is not finite or is outside the range.</exception>
        public byte[] Render(double value)
        {
            if (double.IsNaN(value) || double.IsInfinity(value) || value < Min || value > Max)
            {
                throw new ArgumentOutOfRangeException("value", "double value is not finite or is outside "
                    + RenderFinite(Min) + " to " + RenderFinite(Max) + ", and would not read back");
            }
            return CodecText.Ascii(RenderFinite(value));
        }

        /// <summary>Bitwise, so -0.0 and 0.0 differ.</summary>
        public bool Equal(double a, double b)
        {
            return Bits(a) == Bits(b);
        }

        private string RangeExpectation()
        {
            return "expected a number from " + RenderFinite(Min) + " to " + RenderFinite(Max);
        }

        internal static long Bits(double value)
        {
            return BitConverter.DoubleToInt64Bits(value);
        }

        // False for a number too large for a double on .NET Framework, which throws
        // OverflowException; newer runtimes return infinity, which is outside every range. A
        // text starting '-' that reads as zero is -0, which .NET Framework reads as +0.
        private static bool TryRead(string text, out double value)
        {
            try
            {
                value = double.Parse(text, NumberStyles.Float, CultureInfo.InvariantCulture);
            }
            catch (OverflowException)
            {
                value = 0.0;
                return false;
            }
            if (value == 0.0 && text[0] == '-') value = NegativeZero;
            return true;
        }

        internal static string RenderFinite(double value)
        {
            if (Bits(value) == Bits(NegativeZero)) return "-0.0";
            return CodecText.ChooseFloatText(17,
                precision => value.ToString("G" + CodecText.Number(precision), CultureInfo.InvariantCulture),
                text =>
                {
                    double read;
                    return TryRead(text, out read) && Bits(read) == Bits(value);
                });
        }
    }
}
